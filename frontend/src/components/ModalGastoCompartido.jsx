  ...
  useEffect(() => {
    const cargarUsuariosFusionados = async () => {
      ...
      try {
        ...
        // Fallback a todos los usuarios si hay error
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!(usuarioId in participantesConMonto) || parseFloat(participantesConMonto[usuarioId]) <= 0) {
        alert('⚠️ Debes incluirte a ti mismo en el gasto compartido con un monto mayor a 0');
        return;
      }
            ...
            {loadingFusionados ? (
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
                ...
                <p className="text-xs text-yellow-600 mt-1">
                  Ve a "Fusión de Cuentas" para conectar con otros usuarios y poder crear gastos compartidos
                </p>
              </div>
            ) : (
